...
import { BookOpen, Clock, PlayCircle, Filter, Search } from 'lucide-react';
import { collection, getDocs, onSnapshot, query, where } from 'firebase/firestore';
import { db, auth } from '../lib/firebase';
import { getCategoryBadgeStyle } from '../components/ExamManager';
import { type ExamSummary, toSortedExamList } from '../lib/studentFlow';
  ...
  useEffect(() => {
    ...
    const subscribeTo = async (activeQuery: any) => {
      try {
        ...
        const initialExams = toSortedExamList(initialSnap);
        if (!isMounted) return;
        ...
        if (isAuthenticated) {
          try {
            ...
            const fallbackExams = toSortedExamList(fallbackSnap);
            if (!isMounted) return;
      ...
      unsub = onSnapshot(
        ...
        async (examSnap) => {
          try {
            const nextExams = toSortedExamList(examSnap);
            if (!isMounted) return;
            setExams(nextExams);
            setError(null);
          } catch (err: any) {
