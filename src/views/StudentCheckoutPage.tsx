...
function normalizePlanCode(value: string | null) {
  if (value === 'vip_6m' || value === 'vip_1y' || value === 'vip_1m') return value;
  return 'vip_1m';
}
...
export function StudentCheckout() {
  ...
  const planCode = useMemo(() => normalizePlanCode(new URLSearchParams(location.search).get('plan')), [location.search]);
